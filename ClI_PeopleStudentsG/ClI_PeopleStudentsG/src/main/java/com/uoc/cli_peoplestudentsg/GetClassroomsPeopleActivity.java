...
public class GetClassroomsPeopleActivity extends Activity {
    ...
    private class LoadClassroomTask extends AsyncTask<String, Void, UserList> {
        ...
        protected UserList doInBackground(String... param) {
            /* UOCAPICALL /api/v1/classrooms/{id}/people/students GET*/
            String usl;
            HttpClient httpClient = new DefaultHttpClient();
            HttpGet httpGet = new HttpGet("http://oslo.uoc.es:8080/webapps/uocapi/api/v1/classrooms/" + param[1] + "/people/students"+"?access_token="+param[0]);
            httpGet.setHeader("content-type", "application/json");
            try {
                HttpResponse resp = httpClient.execute(httpGet);
                usl = EntityUtils.toString(resp.getEntity());
            } catch (Exception ex) {
                Log.e("REST", "GET Error!", ex);
                return null;
            }
            return new Gson().fromJson(usl, UserList.class);
            //OR return UserList.getStudentsfromClassRoomWS(param[0], param[1]); if you use our library
        }
    }
/*ENDUOCAPIEXAMPLE*/
