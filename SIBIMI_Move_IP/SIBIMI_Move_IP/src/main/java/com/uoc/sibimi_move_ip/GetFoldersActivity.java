package com.uoc.sibimi_move_ip;
...
public class GetFoldersActivity extends Activity {
    ...
    private class LoadClassroomTask extends AsyncTask<String, Void, FolderList> {
        ...
        protected FolderList doInBackground(String... param) {
            return FolderList.getFolderListfromSubjectBoardWS(param[0], param[1], param[2]);
        }
    }
    ...
    private class MoveToFolderTask extends AsyncTask<String, Void, Folder> {
        ...
        protected Folder doInBackground(String... token) {
            /* UOCAPICALL /api/v1/subjects/{domain_id}/boards/{board_id}/messages/{id}/move/{folder_id} POST*/
            String faux;
            HttpClient httpClient = new DefaultHttpClient();
            HttpPost httpPost = new HttpPost("http://oslo.uoc.es:8080/webapps/uocapi/api/v1/subjects/"+Cid+"/boards/"+Bid+"/messages/"+Mid+"/move/"+f.getId() +"?access_token="+ Utils.getToken());
            try {
            ...
            //OR return Folder.postMoveMessagetoSubjectBoarFolderWS(Utils.getToken(), Cid, Bid, Mid, f.getId()); if you use our library
        }
    }
    /*ENDUOCAPIEXAMPLE*/
}