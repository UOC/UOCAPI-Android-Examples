...
public class GetClassroomsBoardsActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        listview.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, final View view,
                                    int pos, long id) {
                b = listB.get(pos);
                Intent newintent = new Intent (getApplicationContext(), ViewBoardActivity.class);
    ...
    private class LoadClassroomTask extends AsyncTask<String, Void, BoardList> {
        ...
        protected BoardList doInBackground(String... param) {
            ...
            HttpGet httpGet = new HttpGet("http://oslo.uoc.es:8080/webapps/uocapi/api/v1/subjects/"+param[1]+"/boards"+"?access_token="+param[0]);
            httpGet.setHeader("content-type", "application/json");
            try {
                HttpResponse resp = httpClient.execute(httpGet);
                bl = EntityUtils.toString(resp.getEntity());
            } catch (Exception ex) {
                Log.e("REST", "GET Error!", ex);
                return null;
            }
            return new Gson().fromJson(bl, BoardList.class);
            //OR return BoardList.getBoardListfromSubjectWS(param[0], param[1]); if you use our library
        }
    }
    /*ENDUOCAPIEXAMPLE*/
    @Override
