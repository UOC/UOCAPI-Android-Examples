...
public class ViewClassroomsActivity extends Activity{
    ...
    private class LoadEventTask extends AsyncTask<String, Void, Classroom> {
        ...
        protected Classroom doInBackground(String... param) {
            ...
            HttpGet httpGet = new HttpGet("http://oslo.uoc.es:8080/webapps/uocapi/api/v1/subjects/"+param[1]+"?access_token="+param[0]);
            httpGet.setHeader("content-type", "application/json");
            try {
                HttpResponse resp = httpClient.execute(httpGet);
                c = EntityUtils.toString(resp.getEntity());
            } catch (Exception ex) {
                Log.e("REST", "GET Error!", ex);
                return null;
            }
            return new Gson().fromJson(c, Classroom.class);
            //OR return Classroom.getClassroomfromSubjectWS(param[0],param[1]); if you use our library
        }
    }
    /*ENDUOCAPIEXAMPLE*/
    @Override
