...
public class ViewProfileActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        params[1] = myIntent.getStringExtra("PID"); //TODO: Put real person id to search
    ...
    private class LoadEventTask extends AsyncTask<String, Void, Profile> {
        ...
        protected Profile doInBackground(String... param) {
            /* UOCAPICALL /api/v1/people/{id}/profiles/current GET*/
            String pro;
            HttpClient httpClient = new DefaultHttpClient();
            HttpGet httpGet = new HttpGet("http://oslo.uoc.es:8080/webapps/uocapi/api/v1/people/"+param[1]+"/profiles/current"+"?access_token="+param[0]);
            httpGet.setHeader("content-type", "application/json");
            try {
                HttpResponse resp = httpClient.execute(httpGet);
                pro = EntityUtils.toString(resp.getEntity());
            } catch (Exception ex) {
                Log.e("REST", "GET Error!", ex);
                return null;
            }
            return new Gson().fromJson(pro, Profile.class);
            //OR return Profile.getPersonProfileWS(param[0], param[1]); if you use our library
        }
    }
/*ENDUOCAPIEXAMPLE*/
