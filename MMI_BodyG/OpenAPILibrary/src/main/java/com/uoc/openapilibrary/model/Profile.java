...
public class Profile {
	...
	public static Profile getProfileWS(String token) {
		return JSONtoProfile(RESTMethod
				.Get(Constants.BASEURI +"user/profiles/current",
						token));
	}

	public static Profile JSONtoProfile(String profileJSON) {
		return new Gson().fromJson(profileJSON, Profile.class);
	}

	public static Profile putProfileWS(String token, Profile newProfile) {
		return JSONtoProfile(RESTMethod
				.Put(Constants.BASEURI +"user/profiles/current",
						token, Utils.toJSON(newProfile)));
	}
	
	public static Profile getPersonProfileWS(String token, String person_id) {
		return JSONtoProfile(RESTMethod
