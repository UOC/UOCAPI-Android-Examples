...
public class GetClassroomsActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        listview.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, final View view,
                                    int pos, long id) {
                c = listC.get(pos);
                Intent newintent = new Intent (getApplicationContext(), ViewClassroomsActivity.class);
    ...
    private class LoadClassroomTask extends AsyncTask<String, Void, ClassroomList> {
        ...
        protected ClassroomList doInBackground(String... param) {
            ...
            HttpGet httpGet = new HttpGet("http://oslo.uoc.es:8080/webapps/uocapi/api/v1/subjects"+"?access_token="+param[0]);
            httpGet.setHeader("content-type", "application/json");
            try {
                HttpResponse resp = httpClient.execute(httpGet);
                cl = EntityUtils.toString(resp.getEntity());
                Log.i("CLASSROOM", cl.toString());
            } catch (Exception ex) {
                Log.e("REST", "GET Error!", ex);
                return null;
            }
            return new Gson().fromJson(cl, ClassroomList.class);
            //OR return ClassroomList.getClassroomListfromSubjectsWS(param[0]); if you use our library
        }
    }
    /*ENDUOCAPIEXAMPLE*/
    @Override
