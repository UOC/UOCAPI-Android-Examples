...
public class GetFoldersActivity extends Activity {
    ...
    private class MoveToFolderTask extends AsyncTask<String, Void, Folder> {
        ...
        protected Folder doInBackground(String... token) {
            ...
            HttpPost httpPost = new HttpPost("http://oslo.uoc.es:8080/webapps/uocapi/api/v1/subjects/"+Cid+"/boards/"+Bid+"/folders/"+OFid+"/messages/"+Mid+"/move/"+f.getId() +"?access_token="+ Utils.getToken());
            try {
            ...
            //OR return Folder.postChangeMessagetoSubjectBoarFoldertoFolderWS(Utils.getToken(), Cid, Bid, OFid, Mid, f.getId()); if you use our library
        }
    }
    /*ENDUOCAPIEXAMPLE*/
}